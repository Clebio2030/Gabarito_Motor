...
function findIsql() {
  ...
  const fallback = spawnSync('where', ['isql.exe'], { encoding: 'utf8' });
...
function runViewsSql(projectRoot) {
  const envFile = path.join(projectRoot, 'backend', '.env');
  const sqlFile = path.join(projectRoot, 'sql', 'criar_views_gabarito.sql');

  if (!fs.existsSync(envFile)) {
    throw new Error(`Arquivo .env não encontrado em ${envFile}`);
  }

  if (!fs.existsSync(sqlFile)) {
    throw new Error(`Script SQL não encontrado em ${sqlFile}`);
  }

  const env = parseEnvFile(envFile);
  const isql = findIsql();
  if (!isql) {
    throw new Error('isql.exe não encontrado para aplicar as views do Firebird.');
  }

  const host = env.FB_HOST || 'localhost';
  const port = env.FB_PORT || '3050';
  const database = env.FB_DATABASE;
  const user = env.FB_USER || 'SYSDBA';
  const password = env.FB_PASSWORD || 'masterkey';
  const charset = env.FB_CHARSET || 'WIN1252';

  if (!database) {
    throw new Error('FB_DATABASE não está preenchido no backend/.env');
  }

  log('Executando script SQL das views...');
  ...
  const result = spawnSync(isql, args, {
    encoding: 'utf8'
  });
